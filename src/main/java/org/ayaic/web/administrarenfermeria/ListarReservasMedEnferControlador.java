...
@Controller
public class ListarReservasMedEnferControlador {

    private final MiFacade mi;

    public ListarReservasMedEnferControlador(MiFacade mi) {
        this.mi = mi;
    }

    @RequestMapping("/ListarReservasMedEnfer.do")
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        Personas personaaisg = new Personas();
        personaaisg.setId_persona(cliente.getId_persona());
        personaaisg.setCod_esta(cliente.getCod_esta());
        personaaisg.setDip("A");  ////getListaMedicoAsig
        List buscarmed = this.mi.getListaMedicoAsig(personaaisg);
        modelo.put("listamedico", buscarmed);

        if (("".equals(id_persona) || id_persona == null)) {
            id_persona = "0";
        }

        if (!buscarmed.isEmpty() && "0".equals(id_persona)) {
            Personas datoper = (Personas) buscarmed.get(0);
            id_persona = Integer.toString(datoper.getId_persona());
        }

        Historiales dator = new Historiales();
        dator.setCod_esta(cliente.getCod_esta());
        dator.setId_persona(cliente.getId_persona());
        dator.setId_por(Integer.parseInt(id_persona));
        dator.setFecha_ini(dFechaini1);
        dator.setFecha_fin(dFechafin1);
        dator.setId_estado("%");
        dator.setAccion("E");    ////getListarReservacionesAsig
        List listarAsig = this.mi.getListarReservacionesAsig(dator);
        modelo.put("milista", listarAsig);

        if ("Asignar".equals(accion)) {
            String id_medico = request.getParameter("id_medico");
            String accion2 = request.getParameter("accion2");

            if ("adicion".equals(accion2)) {
                Personas persona = new Personas();
                persona.setId_persona(cliente.getId_persona());
                persona.setId_medico(Integer.parseInt(id_medico));
                persona.setCod_esta(cliente.getCod_esta());
                this.mi.setCrearPersonaEnfer(persona);
            }

            if ("eliminar".equals(accion2)) {
                Personas eliminar = new Personas();
                eliminar.setId_persona(cliente.getId_persona());
                eliminar.setId_medico(Integer.parseInt(id_medico));
                eliminar.setCod_esta(cliente.getCod_esta());
                this.mi.setEliminarPersonaAsig(eliminar);
            }

            if (!"0".equals(id_consultorio) && !"".equals(id_consultorio) && request.getParameter("id_consultorio") != null) {
                Personas persona = new Personas();
            ...
            personaaisg.setDip("A");  ////getListaMedicoAsig
            List buscarmed2 = this.mi.getDatosPersonaConsul(personaaisg);
            modelo.put("listamedico", buscarmed2);
            modelo.put("accion", accion);
            modelo.put("nombres", cliente.getNombres());
            modelo.put("id_consultorio", id_consultorio);
            return new ModelAndView("administrarenfermeria/AsignarMedicoEnfer", modelo);
        }
        ...
        if (!"".equals(id_consultorio) && request.getParameter("id_consultorio") != null) {
            ...
            if (request.getParameter("id_persona") != null && !"0".equals(id_persona)) {
                ...
                if ("CambiarConsultorio".equals(accion)) {
                    ...
                    List listarPaises2 = this.mi.getListarReservaciones(dato);
            ...
            List listarPaciCountMed = this.mi.getListarReservacionesCountMed(dato); /////getListarReservacionesCountMed
            modelo.put("milistaCount", listarPaciCountMed);
            return new ModelAndView("administrarenfermeria/ListarReservasMedEnfer", modelo);
        }
