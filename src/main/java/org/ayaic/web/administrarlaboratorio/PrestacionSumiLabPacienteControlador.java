...
@Controller
public class PrestacionSumiLabPacienteControlador {

    private final MiFacade mi;

    public PrestacionSumiLabPacienteControlador(MiFacade mi) {
        this.mi = mi;
    }

    @RequestMapping("/LabSumiPaciente.do")
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        datola.setId_cuaderno(Integer.parseInt(id_reservacion));
        datola.setCod_esta(cliente.getCod_esta());
        Cuadernos datoLab = this.mi.getDatosLaboratorioC(datola);
        //Cuadernos datoLab= this.mi.getDatosLaboratorioC(Integer.parseInt(id_reservacion)) ;  
        ...
        if ("adicion".equals(accion)) {
            ...
            if (listaPre.isEmpty()) {
                ...
                dato.setFecha_ini(fecha);
                dato.setCantidad(1);
                if (!"".equals(canti) && canti != null) {
        ...
        modelo.put("sw", sw);
        return new ModelAndView("administrarlaboratorio/PrestacionSumiPaciente", modelo);
    }
}
