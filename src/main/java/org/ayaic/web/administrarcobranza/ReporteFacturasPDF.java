...
public class ReporteFacturasPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws DocumentException, BadElementException, IOException {
        ...
        Paragraph p = new Paragraph("REPORTE FACTURAS\nEMITIDAS", new Font(Font.HELVETICA, 14, Font.BOLDITALIC, new Color(0, 0, 0)));
        ...
        int[] fwidths = {6, 18, 22, 13, 16, 21, 5, 7, 48, 15};
        table.setWidths(fwidths);
        table.setWidthPercentage(100);

        String sCampos[] = {"No", "FECHA", "RUBRO", "Numero\nFactura", "NIT", "Codigo\nControl", "ESt", "Id", "NOMBRE", "COSTO TOTAL"};
        ...
        for (int i = 0; i < lFopos.size(); i++) {
            ...
            cell = new PdfPCell(new Phrase(format(datopac.getFec_registro(), "dd/MM/yy HH:mm"), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            table.addCell(cell);

            for (int j = 0; j < lRubros.size(); j++) {
                Costos lrubros = (Costos) lRubros.get(j);
                if (lrubros.getId_rubro() == datopac.getId_rubro()) {
                    cell = new PdfPCell(new Phrase((lrubros.getRubro().length() > 12) ? lrubros.getRubro().substring(0, 12) : lrubros.getRubro(), DATO_FONT));
                    cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                    table.addCell(cell);
                }
            }

            cell = new PdfPCell(new Phrase(Integer.toString(datopac.getId_pais()), DATO_FONT_A));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table.addCell(cell);
            ...
            cell = new PdfPCell(new Phrase(datopac.getId_estado(), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table.addCell(cell);
            ...
            if ("V".equals(datopac.getId_estado())) {
                ...
                cell = new PdfPCell(new Phrase("0", DATO_FONT_R));
                cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
                table.addCell(cell);
            }
