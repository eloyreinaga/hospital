...
public class VerResumenXmedicoPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if (listaremi.size() % 25 == 0) {
            una = 0;
        } else {
            una = 1;
        }

        for (int pag = 0; pag < listaremi.size() / 25 + una; pag++) {

            Image escudo = Image.getInstance("/opt/imagenes/lescudo.bmp");
            Paragraph p = new Paragraph("RESUMEN RECETAS DISPENSADAS\nSEGUN MEDICO", new Font(Font.HELVETICA, 14, Font.BOLDITALIC, new Color(0, 0, 0)));
            ...
            cell = new PdfPCell(new Phrase("                        Fecha Reporte :   " , DATO_FONT));///+ fecha.substring(8, 10) + " / " + fecha.substring(5, 7) + " / " + fecha.substring(0, 4) + "   al   " + fecha2.substring(8, 10) + " / " + fecha2.substring(5, 7) + " / " + fecha2.substring(0, 4)
            cell.setBorder(Rectangle.NO_BORDER);
            table1.addCell(cell);
            ...
            int[] fwidths = {7, 70, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10};
            ...
            for (int j = pag * 25 + 0; j < pag * 25 + 25 && j < listaremi.size(); j++) {
