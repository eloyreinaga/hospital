...
public class ListarRecetaIntPDF extends AbstractPdfView {
    ...
    protected void buildPdfMetadata(Map model, Document document, HttpServletRequest request) {
        ...
        document.setMargins(20, 20, 10, 5);
    }
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        Paragraph p = new Paragraph("RECETARIO / RECIBO\nATENCION PACIENTE INTERNADO", new Font(Font.HELVETICA, 14, Font.BOLDITALIC, new Color(0, 0, 0)));
        ...
        cell = new PdfPCell(new Phrase("No.:  " + datoPaciente.getNit(), DATO_FONT_A));
        cell.setHorizontalAlignment(Element.ALIGN_CENTER);
        cell.setBorder(Rectangle.NO_BORDER);
        cell.setColspan(2);
        tablex.addCell(cell);
        ...
        if ("S".equals(datoHis.getExpedido())) {
            cell = new PdfPCell(new Phrase("SPS", DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table1.addCell(cell);
        } else if ("P".equals(datoHis.getExpedido())) {
            cell = new PdfPCell(new Phrase("PROG", DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table1.addCell(cell);
        } else {
            cell = new PdfPCell(new Phrase(".", DATO_FONT));
        ...
        int[] fxxwidths = {4, 11, 50, 20, 6, 6, 6, 6}; // percentage
        ...
        cell = new PdfPCell(new Phrase("DIAGNOSTICO       :  " + datoHis.getCodigo() + "    " + datoHis.getDiagnostico(), DATO_FONT_A));
        cell.setColspan(8);
        table.addCell(cell);
        ...
        cell = new PdfPCell(new Phrase("INDICACIONES\n(Cantidad, Frecuencia, Tiempo uso y Via Adm.)", DATO_FONT));
        cell.setHorizontalAlignment(Element.ALIGN_CENTER);
        table.addCell(cell);
        ...
        for (int i = 0; i < listaKardex.size(); i++) {
            ...
            for (int j = 0; j < listaRecetas.size(); j++) {
                ...
                if (datoReceta.getId_receta() == datoKardex.getId_detalle() && datoReceta.getId_medicamento() == datoKardex.getId_medicamento()) {

                    cell = new PdfPCell(new Phrase(format(datoKardex.getFecha(), "dd/MM/yy HH:mm"), DATO_FONT));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    table.addCell(cell);

                    cell = new PdfPCell(new Phrase(((datoKardex.getMedicamento().length() > 25) ? datoKardex.getMedicamento().substring(0, 25) : datoKardex.getMedicamento()) + "<" + datoKardex.getForma_far() + ">" + datoKardex.getConcentra(), DATO_FONT));
                    cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                    table.addCell(cell);

                    cell = new PdfPCell(new Phrase(datoKardex.getIndicacion() + " [" + datoKardex.getDosifica() + " dias]", DATO_FONT));
                    cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                    table.addCell(cell);
