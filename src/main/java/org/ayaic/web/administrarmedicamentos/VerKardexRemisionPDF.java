...
public class VerKardexRemisionPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        String nombmed = (String) model.get("nombmed");
        ...
        for (int pag = 0; pag < listaremi.size() / 30 + una; pag++) {
            ...
            Paragraph p = new Paragraph("REMISION DE MEDICAMENTOS", new Font(Font.HELVETICA, 14, Font.BOLDITALIC, new Color(0, 0, 0)));
            ...
            int[] fxwidths = {10, 15, 70, 15}; // percentage
            ...
            cell = new PdfPCell(new Phrase("Fecha Reporte :   " + fecha.substring(8, 10) + " / " + fecha.substring(5, 7) + " / " + fecha.substring(0, 4) + "  " + fecha.substring(11, 13) + ":" + fecha.substring(14, 16) + "        al       " + fecha2.substring(8, 10) + " / " + fecha2.substring(5, 7) + " / " + fecha2.substring(0, 4) + "  " + fecha2.substring(11, 13) + ":" + fecha2.substring(14, 16), DATO_FONT));
            cell.setBorder(Rectangle.NO_BORDER);
            table1.addCell(cell);
            ...
            String sCampos[] = {"Nro", "Codigo", "Recetas", "Cant.", "Recetas", "Cant.", "Recetas", "Cant.", "Recetas", "Cant.", "Recetas", "Cant.", "Recetas", "Cant.", "Recetas", "Cant.", "Recetas", "Cant.", "Recetas", "Cant."};
            // coloca la cabecera de titulos
            ...
            for (int j = pag * 30 + 0; j < pag * 30 + 30 && j < listaremi.size(); j++) {
                ...
                if (datoremi.getMedicamento().length() > 27) {
                    nombmed = datoremi.getMedicamento().substring(0, 27);
                } else {
                    nombmed = datoremi.getMedicamento();
                }
                cell = new PdfPCell(new Phrase(datoremi.getCodsumi() + "   " + nombmed + " " + datoremi.getForma_far(), DATO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                table.addCell(cell);
            ...
            cb.addImage(escudo, 50, 0, 0, 60, 20, 525, true);
