...
public class FacturaPPagoPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws BadElementException, IOException, DocumentException {
        ...
        String nombreusua = (String) model.get("nombres");
        ...
        double sumatot = (Double.parseDouble((String) model.get("valortotal")));
        ...
        cod = destab.getNit() + '|' + destab.getNum_fact() + '|' + destab.getNum_auto() + '|' + format(new Date(), "dd/MM/yyyy") + '|' + sumatot + '|' + sumatot + '|' + codigo + '|' + ciu + '|' + '0' + '|' + '0' + '|' + '0' + '|' + '0';
        ...
        finalImage.setAbsolutePosition(497, 405);
        ...
        cell = new PdfPCell(new Phrase("A Cuenta Plan de Pagos", DATO_FONT_A));
        cell.setHorizontalAlignment(Element.ALIGN_LEFT);
        ...
        if (costo != null) {
            for (int j = 0; j < costo.size(); j++) {
                ...
                cell = new PdfPCell(new Phrase(df.format(datoc.getCosto_unit()), DATO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setBorder(Rectangle.NO_BORDER);
                table11.addCell(cell);
