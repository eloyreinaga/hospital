...
public class ListarRecetaCajaIntPDF extends AbstractPdfView {

    private static final Font TITULO_FONT = new Font(Font.HELVETICA, 11, Font.BOLD, Color.red);
    private static final Font CABEZA_COLUMNA_FONT = new Font(Font.HELVETICA, 8, Font.ITALIC, Color.red);
    private static final Font DATO_FONT = new Font(Font.HELVETICA, 7, Font.NORMAL, Color.red);
    private static final Font DATO_FONT_B = new Font(Font.HELVETICA, 7, Font.NORMAL, Color.red);
    private static final Font DATO_FONT_D = new Font(Font.HELVETICA, 18, Font.NORMAL, Color.red);
    private static final Font DATO_FONT_F = new Font(Font.HELVETICA, 38, Font.NORMAL, Color.red);
    private static final Font DATO_FONT_A = new Font(Font.COURIER, 10, Font.NORMAL, Color.red);
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        for (int i = 0; i < listaRecetas.size(); i++) {
            ...
            if (datoReceta.getId_historia() == Integer.parseInt(inter)) {
                ...
                cell = new PdfPCell(new Phrase("    ", DATO_FONT_F));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setColspan(3);
                ...
                cell = new PdfPCell(new Phrase("fecha  : " + format(new Date(), "dd/MM/yyyy") + "  " + format(new Date(), "HH:mm"), TITULO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setColspan(3);
                table51.addCell(cell);

                cell = new PdfPCell(new Phrase("", TITULO_FONT));//////alta
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setBorder(Rectangle.NO_BORDER);
                cell.setColspan(2);
                table51.addCell(cell);
                ...
                cell = new PdfPCell(new Phrase("Clv Unid\nSanitaria", DATO_FONT_A));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setBorder(Rectangle.NO_BORDER);
                tablex.addCell(cell);
                ...
                Historiales datoHisInt = (Historiales) listaHisInt.get(listaHisInt.size() - 1);

                if (datoInterna != null) {
                    cell = new PdfPCell(new Phrase("\nServicio de: " + datoInterna.getCodigo() + "  Cama : " + datoInterna.getAccion(), TITULO_FONT));
                } else {
                    cell = new PdfPCell(new Phrase("\nServicio de: " + datoHisInt.getConsultorio() + "  Piso : " + datoHisInt.getCodigo() + "  Cama : " + datoHisInt.getAccion(), TITULO_FONT));
                }
                ...
                if (dato.getId_almacen() == 1) {/////para internaciones 25/08/2015
                    ...
                    ColumnText.showTextAligned(writer.getDirectContentUnder(), Element.ALIGN_RIGHT,
                            new Paragraph(datoMed.getConsultorio().toUpperCase(), marcagua), 460, 380, 90);    ///marca de agua
                }
