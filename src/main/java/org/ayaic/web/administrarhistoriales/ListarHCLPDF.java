...
public class ListarHCLPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        cell = new PdfPCell(new Phrase("INSTRUCTIVO:\n" + "\nSUBJETIVO:   Motivo de consulta y/o sintomas que el paciente refiere durante la anamnesis\n" + "\nOBJETIVO:   Hallazgos del examen fï¿½sico y/o resultados de exï¿½menes de laboratorio y complementarios.\n" + "\nANoLISIS:   Lista de problemas detectados: diagnï¿½stico, signos o sintomas a seguir, resultados de laboratorio patolï¿½gico, antecedentes personales.\n" + "\nPLAN DE ACCION:   Tratamientos, orientaciones, seguimientos, exï¿½menes complementarios necesarios para cada problema.\n", DATO_FONT));
        ...
        for (int pag = 0; pag < HCL.size() / filas + una; pag++) {
            ...
            for (int i = pag * filas + 0; i < pag * filas + filas && i < HCL.size(); i++) {
                ...
                ////cell.setBorder(Rectangle.NO_BORDER);
                ////table12.addCell(cell);
                ////cell = new PdfPCell(new Phrase("Paciente:   "+pacientes.getNombres(), new Font( Font.COURIER, 7, Font.NORMAL, Color.black )));
                ////cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                ////cell.setBorder(Rectangle.NO_BORDER);
                ////table12.addCell(cell);
                //// codigo de barras
                ////Barcode128 code128 = new Barcode128();
                ////code128.setCode(datopac.getNombres());
                ////code128.setBarHeight(20f); 
                ////code128.setTextAlignment(0);
                ////code128.setSize(0.5f);///Tamaño letra leyenda
                ////java.awt.Image awtImage = code128.createAwtImage(Color.WHITE, Color.BLACK);
                ////PdfContentByte cb = writer.getDirectContent();
                ////PdfPCell cellBarCode = new PdfPCell(code128.createImageWithBarcode(cb, null, null));
                ////cellBarCode.setHorizontalAlignment(Element.ALIGN_LEFT);        
                ////cellBarCode.setBorder(Rectangle.NO_BORDER);
                ////table12.addCell(cellBarCode);
