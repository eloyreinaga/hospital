...
public class HCLBasicaPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        for (int pag = 0; pag < HCL.size() / filas + una; pag++) {
            ...
            for (int i = pag * filas + 0; i < pag * filas + filas && i < HCL.size(); i++) {
                ...
                cell = new PdfPCell(new Phrase("Paciente:   " + pacientes.getNombres() + "\nHcl : " + pacientes.getHcl() + "    Matricula : " + pacientes.getNro_registro(), new Font(Font.COURIER, 8, Font.NORMAL, Color.black)));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setVerticalAlignment(Element.ALIGN_BOTTOM);
                cell.setBorder(Rectangle.NO_BORDER);
