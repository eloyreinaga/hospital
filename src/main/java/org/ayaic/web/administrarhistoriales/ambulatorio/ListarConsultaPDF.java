...
public class ListarConsultaPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        Personas empleado = (Personas) model.get("empleado");
        Clientes dato = (Clientes) model.get("dato");
        String soloedad = (String) model.get("soloedad");
        String mes = (String) model.get("mes");
        String adm = (String) model.get("tipo_medico");
        ...
        if (sTitulo == null) {
            sTitulo = "Resumen de prestaciones realizadas";
        }
        int una;
        if (lPacientes.size() == 0) {
            Paragraph p = new Paragraph("No existe Datos Ambulatorio", new Font(Font.HELVETICA, 22, Font.BOLDITALIC, new Color(0, 0, 0)));
            document.add(p);
        }
        ...
        for (int pag = 0; pag < lPacientes.size() / 30 + una; pag++) {
            ...
            int[] fwidths = {6, 16, 17, 17, 22, 6, 6, 6, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4}; // percentage
                ...
                for (int j = 0; j < lPrestaciones.size(); j++) {
                    ...
                    for (int k = 0; k < lPacientePres.size(); k++) {
                        ...
                        if (datopac.getHcl() == datopacpre.getHcl() && datopac.getId_historial() == datopacpre.getId_historial()
                                && datopre.getId_prestacion() == datopacpre.getId_prestacion()) { ///datopre.getPrestacion().equals(datopacpre.getPrestacion())) 

                            datopre.setCantidad(datopre.getCantidad() + datopacpre.getCantidad());
                            datopre.setHcl(1);
            ...
            if ("1".equals(dato.getTipo())) {
                cell = new PdfPCell(new Phrase(("1".equals(dato.getTipo())) ? "NIVEL ATEN.: PRIMER NIVEL" : "", DATO_FONT));
                cell.setBorder(Rectangle.NO_BORDER);
                table1.addCell(cell);
            } else {
                if ("2".equals(dato.getTipo())) {
                    cell = new PdfPCell(new Phrase(("2".equals(dato.getTipo())) ? "NIVEL ATEN.: SEGUNDO NIVEL" : "", DATO_FONT));
                    cell.setBorder(Rectangle.NO_BORDER);
                    table1.addCell(cell);
                } else {
                    cell = new PdfPCell(new Phrase(("3".equals(dato.getTipo())) ? "NIVEL ATEN.: TERCER NIVEL" : "", DATO_FONT));
                    cell.setBorder(Rectangle.NO_BORDER);
                    table1.addCell(cell);
                }
            }

            cell = new PdfPCell(new Phrase("Mes : " + meses[Integer.parseInt(mes) - 1], DATO_FONT));
            cell.setBorder(Rectangle.NO_BORDER);
            table1.addCell(cell);

            cell = new PdfPCell(new Phrase("Año : " + annio, DATO_FONT));
            cell.setBorder(Rectangle.NO_BORDER);
            table1.addCell(cell);
            ...
            table1 = new PdfPTable(32);
            int[] fmpwidths = {4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4}; // percentage
            ...
            int[] fxwidths = {6, 16, 17, 17, 22, 6}; // percentage
            ...
            int[] fxxxywidths = {6, 6}; // percentage
            table1.setWidths(fxxxywidths);
            table1.setWidthPercentage(100);
            cell = new PdfPCell(new Phrase("EDAD", DATO_FONT_P));
            cell.setColspan(2);
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table1.addCell(cell);
            ...
            //sexo
            table1 = new PdfPTable(2);
            int[] fxxxwidths = {3, 3}; // percentage
            table1.setWidths(fxxxwidths);
            table1.setWidthPercentage(100);
            cell = new PdfPCell(new Phrase("SEXO", DATO_FONT_P));
            cell.setColspan(2);
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table1.addCell(cell);
            ...
            // consulta
            table1 = new PdfPTable(2);
            int[] fxxwidths = {3, 3}; // percentage
            table1.setWidths(fxxwidths);
            table1.setWidthPercentage(100);

            cell = new PdfPCell(new Phrase("CONSULTA", DATO_FONT_P));
            cell.setColspan(2);
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table1.addCell(cell);
            ...
            for (int i = 0; i < lPrestaciones.size(); i++) {
                Prestaciones datopre = (Prestaciones) lPrestaciones.get(i);
                if (datopre.getHcl() == 1) {
                    cell = new PdfPCell(new Phrase(datopre.getPrestacion(), DATO_FONT_P));
                    cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                    cell.setRotation(90);
                    table.addCell(cell);
                    cont++;
            ...
            for (int i = pag * 30 + 0; i < pag * 30 + 30 && i < lPacientes.size(); i++) {
                ...
                if (datopac.getNombre().length() > 13) {
                ...
                if (datopac.getDia() == datopac.getDial()) {
                    cell = new PdfPCell(new Phrase(Integer.toString(datopac.getDia()), DATO_FONT));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    table.addCell(cell);
                } else {
                    cell = new PdfPCell(new Phrase(Integer.toString(datopac.getDial()), DATO_FONT));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    table.addCell(cell);
                }
                    ...
                    if (datopre.getHcl() == 1) {
                        ...
                        for (int k = 0; k < lPacientePres.size(); k++) {
                            ...
                            if (datopac.getHcl() == datopacpre.getHcl() && datopac.getId_historial() == datopacpre.getId_historial()
                                    && datopre.getId_prestacion() == datopacpre.getId_prestacion()) ///datopre.getPrestacion().equals(datopacpre.getPrestacion()))  
                            {
                                esta = Integer.toString(datopacpre.getCantidad());
                            }
                        }
            ...
            if ("7".equals(adm)) {
                cell = new PdfPCell(new Phrase(empleado.getNombres() + " " + empleado.getPaterno() + " " + empleado.getMaterno(), DATO_FONT));
                ...
                cell = new PdfPCell(new Phrase(dato.getNombres(), DATO_FONT));
                cell.setColspan(NumColumns);
                cell.setBorder(Rectangle.NO_BORDER);
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setVerticalAlignment(Element.ALIGN_BOTTOM);
            ...
            cell = new PdfPCell(new Phrase("Nombre completo del responsable                                      Firma", DATO_FONT));
            cell.setColspan(NumColumns);
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            cell.setBorder(Rectangle.NO_BORDER);
            table.addCell(cell);
