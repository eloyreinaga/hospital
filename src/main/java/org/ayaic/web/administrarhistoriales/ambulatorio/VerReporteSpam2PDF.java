...
public class VerReporteSpam2PDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        Paragraph p = new Paragraph("LISTADO DE PACIENTES EN LABORATORIOS", new Font(Font.HELVETICA, 14, Font.BOLDITALIC, new Color(0, 0, 0)));
        ...
        int[] fwidths = {5, 16, 35, 14, 7, 60, 8};
        table.setWidths(fwidths);
        table.setWidthPercentage(100);

        String sCampos[] = {"No", "FECHA", "NOMBRE DEL PACIENTE", "No SEGURO", "Edad", "LABORATORIOS", "Costo"};
        ...
        for (int i = 0; i < lFopos.size(); i++) {
            ...
            cell = new PdfPCell(new Phrase(format(datopac.getFecha(), "dd/MM/yy HH:mm"), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            table.addCell(cell);
            ...
            for (int j = 0; j < listalab.size(); j++) {
                Cuadernos Listalab = (Cuadernos) listalab.get(j);

                if (datopac.getId_historial() == Listalab.getId_historial() && datopac.getId_pedido() == Listalab.getId_pedido()) {
                    Cadena = Cadena + " " + Listalab.getNombre().trim() + ";";
                    ccc += Listalab.getNumpieza();
                }
            }

            cell = new PdfPCell(new Phrase(Cadena.trim(), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            table.addCell(cell);

            cell = new PdfPCell(new Phrase(Integer.toString(ccc), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
            table.addCell(cell);
