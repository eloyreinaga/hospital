...
public class VerReporteFarmaPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        java.util.List listalab = (java.util.List) model.get("listapago");
        ...
        Paragraph p = new Paragraph("RESUMEN ECONOMICO DE MOVIMIENTO", new Font(Font.HELVETICA, 14, Font.BOLDITALIC, new Color(0, 0, 0)));
        ...
        cell = new PdfPCell(new Phrase("MEDICAMENTOS", TITULO_FONT));
        cell.setHorizontalAlignment(Element.ALIGN_CENTER);
        cell.setBorder(Rectangle.NO_BORDER);
        cell.setColspan(2);
        table1.addCell(cell);
        ...
        int[] fwidths = {6, 9, 30, 6, 60, 6};
        table.setWidths(fwidths);
        table.setWidthPercentage(100);

        String sCampos[] = {"No", "Fecha", "PACIENTE", "EDAD", "MEDICAMENTOS", "Costo"};
        ...
        for (int i = 0; i < listalab.size(); i++) {
            ...
            Pacientes datopac = (Pacientes) listalab.get(i);

            cell = new PdfPCell(new Phrase(Integer.toString(i + 1), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            table.addCell(cell);

            cell = new PdfPCell(new Phrase(format(datopac.getFec_registro(), "dd/MM/yy"), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            table.addCell(cell);
            ...
            cell = new PdfPCell(new Phrase(Integer.toString(datopac.getId_rubro()), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table.addCell(cell);
            ...
            for (int j = 0; j < listaKardex.size(); j++) {
                Recetas ListaK = (Recetas) listaKardex.get(j);

                if (datopac.getId_pedido() == ListaK.getId_pedido()) {
                    Cadena = Cadena + "(" + dfx.format(ListaK.getSalida()) + ")" + " " + ListaK.getMedicamento().trim() + "[" + ListaK.getConcentra() + "]" + "; ";
                    prectot += ListaK.getPrecio_total();
                }
            }

            cell = new PdfPCell(new Phrase(Cadena, DATO_FONT_A));
            cell.setHorizontalAlignment(Element.ALIGN_LEFT);
            table.addCell(cell);

            cell = new PdfPCell(new Phrase(df.format(prectot), DATO_FONT));
            cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
            table.addCell(cell);
        ...
        cell = new PdfPCell(new Phrase("Total Bs    :   " + df.format(total), DATO_FONT));
        cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
        cell.setColspan(6);
        table.addCell(cell);
