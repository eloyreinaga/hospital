...
public class ListarOtrosAfiliadosPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        Paragraph p = new Paragraph("REGISTRO DE AFILIACION ASEGURADOS", new Font(Font.HELVETICA, 14, Font.BOLDITALIC, new Color(0, 0, 0)));
        ...
        int[] fwidths = {4, 7, 8, 10, 10, 10, 6, 2, 2, 2, 2, 25, 10};
        table.setWidths(fwidths);
        table.setWidthPercentage(100);
        String sCampos[] = {"No", "Fecha\nAfiliacion", "H.CL.", "Apellido\nPaterno", "Apellido\nMaterno", "Nombres", "Seguro", "años", "Meses", "M", "F", "Direccion", "Municipio"};
        if ("SUMI NINIO".equals(tipo) || "SUMI MUJER".equals(tipo)) {
            sCampos[2] = "N. de\nRegistro";
            sCampos[6] = "CI/RIN\nRUN";
        }
        ...
        for (int i = 0; i < lAfiliados.size(); i++) {
            ...
            if ("SUMI NINIO".equals(tipo) && datopac.getEdad() < 6) {
                cell = new PdfPCell(new Phrase(Integer.toString(ii++), DATO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                table.addCell(cell);
                ...
                cell = new PdfPCell(new Phrase(dato.getLocalidad(), DATO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                table.addCell(cell);
            }
            if ("SUMI MUJER".equals(tipo) && datopac.getEdad() > 5) {
                cell = new PdfPCell(new Phrase(Integer.toString(ii++), DATO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                table.addCell(cell);
                ...
                cell = new PdfPCell(new Phrase(datopac.getSeguro(), DATO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                table.addCell(cell);
            ...
            if (!"SUMI MUJER".equals(tipo) && !"SUMI NINIO".equals(tipo)) {
                cell = new PdfPCell(new Phrase(Integer.toString(ii++), DATO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                table.addCell(cell);
