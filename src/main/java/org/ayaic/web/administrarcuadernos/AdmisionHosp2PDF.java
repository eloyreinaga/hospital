...
public class AdmisionHosp2PDF extends AbstractPdfView {

    private static final Font TITULO_FONT = new Font(Font.HELVETICA, 12, Font.BOLD, Color.white);
    private static final Font TITULO_FONT_B = new Font(Font.HELVETICA, 16, Font.BOLD, Color.white);
    private static final Font CABEZA_COLUMNA_FONT = new Font(Font.HELVETICA, 10, Font.ITALIC, Color.white);
    private static final Font DATO_FONT = new Font(Font.HELVETICA, 11, Font.NORMAL, Color.white);
    private static final Font DATO_FONT2 = new Font(Font.HELVETICA, 11, Font.NORMAL, Color.black);
    private static final Font DATO_FONT_BOLD = new Font(Font.COURIER, 10, Font.BOLD, Color.white);
    private static final Font marcagua = new Font(Font.HELVETICA, 26, Font.BOLD, Color.white);
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        for (int pag = 0; pag < HCL.size() / filas + una; pag++) {
            ...
            for (int i = pag * filas + 0; i < pag * filas + filas && i < HCL.size(); i++) {
                ...
                cell = new PdfPCell(new Phrase("\n\nNombres :  " + pacientes.getNombre() + "    Paterno :  " + pacientes.getPaterno() + "     Materno :  " + pacientes.getMaterno(), DATO_FONT));
                cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                cell.setBorder(Rectangle.NO_BORDER);
                cell.setColspan(3);
                tablex.addCell(cell);
                ...
                cell = new PdfPCell(new Phrase("\nPersona mas Proxima, Otros Datos: " + datopac.getCadena5(), DATO_FONT2));
                ...
                java.awt.Image qrImage = qrcode.createAwtImage(Color.white, Color.white);
