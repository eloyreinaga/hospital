...
public class ListarRecetaIntPDF extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map model, Document document, PdfWriter writer, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        cell = new PdfPCell(new Phrase(format(new Date(), "dd/MM/yyyy"), DATO_FONT));
        cell.setHorizontalAlignment(Element.ALIGN_CENTER);
        table1.addCell(cell);
        ...
        int[] fxxwidths = {4, 50, 20, 6, 6, 6, 6}; // percentage
        ...
        for (int i = 0; i < listaKardex.size(); i++) {
            ...
            for (int j = 0; j < listaRecetas.size(); j++) {
                ...
                if (datoReceta.getId_receta() == datoKardex.getId_detalle() && datoReceta.getId_medicamento() == datoKardex.getId_medicamento()) {
                    ...
                    cell = new PdfPCell(new Phrase(((datoKardex.getMedicamento().length() > 25) ? datoKardex.getMedicamento().substring(0, 25) : datoKardex.getMedicamento()) + "<" + datoKardex.getForma_far() + ">" + datoKardex.getConcentra(), DATO_FONT_A));
                    cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                    table.addCell(cell);
